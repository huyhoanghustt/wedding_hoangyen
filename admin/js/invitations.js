 ...
 * - admin-guest-links.js (for guest-specific invitation rendering)
 * - admin invitation editor (for format management)
...
const INVITATION_DEFAULTS = {
    ...
    formats: {
        default: {
            ...
            slot_descriptors: [
                { index: 0, label: '{d0}', placeholder: '{d0}', type: 'guest', hint: 'guest pronoun' },
                { index: 1, label: '{d1}', placeholder: '{d1}', type: 'guest', hint: 'Guest (cap)' },
                { index: 2, label: '{d2}', placeholder: '{d2}', type: 'guest', hint: 'guest pronoun' },
                { index: 3, label: '{d3}', placeholder: '{d3}', type: 'couple', hint: 'couple pronoun' },
                { index: 4, label: '{d4}', placeholder: '{d4}', type: 'guest', hint: 'guest pronoun' },
                { index: 5, label: '{d5}', placeholder: '{d5}', type: 'family-companion', hint: "companion after 'và'" },
                { index: 6, label: '{d6}', placeholder: '{d6}', type: 'couple', hint: 'couple pronoun' },
                { index: 7, label: '{d7}', placeholder: '{d7}', type: 'guest', hint: 'guest pronoun' },
                { index: 8, label: '{d8}', placeholder: '{d8}', type: 'couple', hint: 'couple pronoun' }
            ]
        }
    },
    role_presets: {
 ...
 */
function normalizePronounArray(sourcePronouns, defaultPronouns, invitationsData) {
    // Calculate target length dynamically based on slot descriptors
    const targetLength = invitationsData ? getPronounArrayLength(invitationsData) : INVITATION_DEFAULTS.formats.default.default_pronouns.length;
    ...
    // Only apply migration if we're using the standard 9-slot format
    const isStandardFormat = targetLength === 9;
    if (isStandardFormat && rawValues.length === targetLength - 1) {
        const migrated = [
 ...
 * @param {string} formatKey - Optional format key (defaults to active_format)
 * @returns {number} Expected pronoun array length
 */
function getPronounArrayLength(invitationsData, formatKey) {
    const descriptors = getSlotDescriptors(invitationsData, formatKey);
...
/**
 * Get pronoun_for_title default preset by pronoun_for_title value.
 * Looks up the preset from the pronoun_for_title_defaults array in invitations data.
 * @param {Object} invitationsData - The invitations data
 * @param {string} pronounForTitle - The pronoun_for_title value to look up
 * @returns {Object|null} Matching preset object with normalized pronouns, or null if not found
 */
function getPronounForTitleDefaults(invitationsData, pronounForTitle) {
    if (!pronounForTitle) return null;
    
    const key = String(pronounForTitle).trim().toLowerCase();
    if (!key) return null;
    
    const defaults = invitationsData?.pronoun_for_title_defaults;
    if (!Array.isArray(defaults) || defaults.length === 0) return null;
    
    // Find matching preset by normalized key
    const preset = defaults.find(entry => {
        const entryKey = String(entry?.pronoun_for_title || '').trim().toLowerCase();
        return entryKey === key;
    });
    
    if (!preset || typeof preset !== 'object') return null;
    
    // Validate pronouns array exists
    if (!Array.isArray(preset.pronouns)) return null;
    
    return {
        pronoun_for_title: preset.pronoun_for_title,
        label: preset.label || capitalizeFirstWord(preset.pronoun_for_title),
        pronouns: [...preset.pronouns] // Return copy to prevent mutation
    };
}

/**
 * Normalize a pronoun_for_title_defaults entry to match target slot count.
 * @param {Object} entry - The preset entry to normalize
 * @param {number} targetLength - Target pronoun array length
 * @returns {Object} Normalized entry with pronouns array of correct length
 */
function normalizePronounForTitleDefaultsEntry(entry, targetLength) {
    if (!entry || typeof entry !== 'object') return null;
    
    const pronouns = Array.isArray(entry.pronouns) ? entry.pronouns : [];
    const normalized = Array.from({ length: targetLength }, (_, i) => {
        const value = pronouns[i];
        return typeof value === 'string' ? value : '';
    });
    
    return {
        pronoun_for_title: entry.pronoun_for_title,
        label: entry.label || capitalizeFirstWord(entry.pronoun_for_title),
        pronouns: normalized
    };
}

/**
 * Get a map of pronoun_for_title_defaults for O(1) lookup.
 * @param {Object} invitationsData - The invitations data
 * @returns {Map<string, Object>} Map of normalized key -> preset entry
 */
function getPronounForTitleDefaultsMap(invitationsData) {
    const map = new Map();
    
    const defaults = invitationsData?.pronoun_for_title_defaults;
    if (!Array.isArray(defaults)) return map;
    
    defaults.forEach(entry => {
        if (!entry || typeof entry !== 'object') return;
        const key = String(entry.pronoun_for_title || '').trim().toLowerCase();
        if (key && Array.isArray(entry.pronouns)) {
            map.set(key, {
                pronoun_for_title: entry.pronoun_for_title,
                label: entry.label || capitalizeFirstWord(entry.pronoun_for_title),
...
if (typeof window !== 'undefined') {
    window.InvitationHelpers = {
        ...
        getPronounForTitleDefaults,
        normalizePronounForTitleDefaultsEntry,
        getPronounForTitleDefaultsMap,
